...
const SpendingAnalysisChart: React.FC<SpendingAnalysisChartProps> = ({
  ...
  const [chartWidth, setChartWidth] = useState<number>(window.innerWidth - 80);

  useEffect(() => {
    const handleResize = () => {
      // Account for margins and padding: mx-6 (24px each side) + p-4 (16px each side)
      ...
      <div className="h-48">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart
            ...
            margin={{
              top: 5,
              right: platform === "ios" ? 20 : 10,
              left: platform === "ios" ? -10 : -20,
              bottom: 5,
            }}
          >
            <defs>
